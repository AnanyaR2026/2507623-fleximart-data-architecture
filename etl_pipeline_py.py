...
# Load customers
# ------------------------------
# Ensure all emails are unique before insertion using a more robust method
...
# The first occurrence of a duplicate email (cumcount=0) can keep its original email
# Subsequent occurrences (cumcount > 0) need a modification.
...
# Prepare data for Orders table (include original transaction_id for mapping)
...
# Group by transaction_id to create one order per transaction
orders_data_to_insert = valid_transactions_for_orders.groupby('transaction_id').agg(
    ...
    status=('status','first')
).reset_index() # Keep 'transaction_id' as a column for later mapping

# Fix: Convert customer_id to integer type to match the database schema
orders_data_to_insert['customer_id'] = orders_data_to_insert['customer_id'].astype(int)

# Fix: Convert total_amount to float type to ensure compatibility for merging
orders_data_to_insert['total_amount'] = orders_data_to_insert['total_amount'].astype(float)

# Insert orders into the 'orders' table
# Only pass columns that match the 'orders' table schema
...
# Fetch the newly inserted orders from the database, including the auto-generated order_id
# We need to join this back with orders_data_to_insert to get the transaction_id
# This join assumes (customer_id, order_date, total_amount, status) combination is unique enough to map orders.

# Fetch all orders from the database
all_db_orders = pd.read_sql("SELECT order_id, customer_id, order_date, total_amount, status FROM orders", conn)

# Fix: Convert total_amount in all_db_orders to float type for compatibility with orders_data_to_insert
all_db_orders['total_amount'] = all_db_orders['total_amount'].astype(float)

# Merge to link the database-generated order_id back to the original transaction_id
...
# Now, merge transactions_items_prepped with orders_with_db_id to get the actual database-generated 'order_id'
# Both DataFrames have 'transaction_id' which can be used for merging.
...
# Filter out order items that couldn't be linked to an order (where order_id is NaN)
# This will happen for transactions where customer_id was originally NaN.
order_items_to_load = order_items_final_df.dropna(subset=['order_id'])

# Select and rename columns for insertion into order_items table
...
# Ensure 'order_id' is of integer type before inserting, as it's INTEGER in DB
